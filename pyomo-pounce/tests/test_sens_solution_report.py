"""Tests for pyomo_pounce.sens_solution_report: what the linear step does
about the bounds."""
import numpy as np
...
def brute_force_multi(model, perturb):
    ...
    est = sens_solution(model, perturb, clamp=False)
...
def test_step_fraction_matches_the_hand_computed_crossing():
    ...
    m = bounded()
    r = sens_solution_report(m, [(m.p, 4.0)])
    assert r.first == "y"
...
def test_an_interior_perturbation_crosses_nothing():
    m = bounded()
    r = sens_solution_report(m, [(m.p, 1.2)])
    assert r.alpha > 1.0
...
def test_crossed_reports_the_distance_past_the_bound():
    m = bounded()
    r = sens_solution_report(m, [(m.p, 4.0)])
    est = sens_solution(m, [(m.p, 4.0)], clamp=False)
...
def test_a_constraint_row_can_bind_before_any_variable():
    m = with_row()
    r = sens_solution_report(m, [(m.p, 3.0)])
...
def test_violation_matches_direct_evaluation_at_the_predicted_point():
    m = with_row()
    r = sens_solution_report(m, [(m.p, 3.0)])
    for v, val in sens_solution(m, [(m.p, 3.0)], clamp=False).items():
        v.set_value(val)
...
def test_an_already_active_bound_is_not_a_crossing():
    ...
    r = sens_solution_report(m, [(m.p, 5.0)])
...
def test_a_saturating_control_is_named_with_its_step_fraction():
    ...
    to drive a control onto its bound, where sens_solution() clamps and
    ...
    assert sens_solution_report(m, [(m.sp, fits)]).alpha == pytest.approx(
        1.0, rel=1e-6)
...
def test_a_weakly_active_bound_is_classified_as_such():
    ...
    r = sens_solution_report(m, [(m.p, 0.5)])
...
def test_several_parameters_perturbed_at_once():
    ...
    r = sens_solution_report(m, [(m.p, 2.0), (m.q, 4.0)])
    assert r.first == "y"
    assert r.alpha == pytest.approx(0.5, abs=1e-6)
...
def test_every_solver_route_reports_the_same():
    ...
    for name, solve in routes:
        ...
        declare_sens_param(m.p)
        solve(m)
        reports.append(sens_solution_report(m, [(m.p, 4.0)]))
...
def test_a_bound_the_step_crosses_is_never_missed():
    ...
    for m, newval in cases:
        r = sens_solution_report(m, [(m.p, newval)])
...
def test_a_coordinate_on_one_bound_can_still_cross_the_other():
    ...
    assert sens_solution_report(m, [(m.p, 1.1)]).activity["x"] == "weakly_active"
...
def test_a_coordinate_pushed_further_past_the_bound_it_is_held_at():
    ...
    m = held_at_a_bound()
    r = sens_solution_report(m, [(m.p, 1.5)], degeneracy="one_sided")
    assert r.activity["x"] == "weakly_active"
    ...
    assert len(sens_solution_report(m, [(m.p, 1.5)]).crossed) == 0

    # the classification still keeps it out when nothing crosses, which
    # is what the exclusion is for
    r2 = sens_solution_report(m, [(m.p, 1.0 + 1e-9)], degeneracy="one_sided")
...
def test_a_loose_solve_does_not_widen_the_floor_past_a_real_gap(opts, label):
    ...
    r = sens_solution_report(m, [(m.p, 1002.0)])
...
def test_max_iter_here_warns_instead_of_being_ignored():
    ...
    with pytest.warns(DeprecationWarning, match="max_iter no longer"):
        got = sens_solution_report(m, [(m.p, 4.0)], max_iter=0)
    ...
    # to force
    assert got.alpha == pytest.approx(sens_solution_report(m, [(m.p, 4.0)]).alpha)


def test_the_other_two_surfaces_keep_their_budget_silent():
    """`sens_solution` and `sens_active_set_changes` still spend a budget
    ...
    with _w.catch_warnings(record=True) as caught:
        ...
        sens_solution(m, [(m.p, 1.2)], predictor_iter=8)
        sens_active_set_changes(m, [(m.p, 1.2)], predictor_iter=8)
...
def test_the_violation_is_the_one_at_the_mode_s_own_point(mode):
    """The report has to describe the step `sens_solution()` takes for the
    same arguments. Evaluating the constraint at that mode's estimate
    is what says whether it does."""
    m = coupled()
    r = sens_solution_report(m, [(m.p, -1.2)], mode=mode)
    for v, val in sens_solution(m, [(m.p, -1.2)], clamp=False, mode=mode).items():
        v.set_value(val)
    body = pyo.value(m.x) * pyo.value(m.y)
    rhs = 1.0 + 0.4 * (-1.2)
...
def test_the_violation_moves_with_the_mode():
    m = coupled()
    lin = sens_solution_report(m, [(m.p, -1.2)], mode="linear").violation
    fix = sens_solution_report(m, [(m.p, -1.2)], mode="fix_relax").violation
    pat = sens_solution_report(m, [(m.p, -1.2)], mode="path").violation
    assert lin > 5 * fix, (
...
def test_the_other_modes_report_a_step_that_stops_at_the_bound():
    ...
    m = coupled()
    assert sens_solution_report(m, [(m.p, -1.2)]).crossed, (
...
def test_the_base_point_fields_do_not_move_with_the_mode():
    m = coupled()
    reports = [sens_solution_report(m, [(m.p, -1.2)], mode=mode)
...
def test_the_corrector_measures_the_mode_s_own_step():
    ...
    m = coupled()
    lin = sens_solution_report(m, [(m.p, -1.2)], mode="linear", corrector_iter=6)
    fix = sens_solution_report(m, [(m.p, -1.2)], mode="fix_relax", corrector_iter=6)
...
def test_an_unknown_mode_is_refused():
    ...
    with pytest.raises(ValueError, match="mode must be"):
        sens_solution_report(m, [(m.p, 2.0)], mode="relax_fix")
...
def test_refine_stop_says_why_the_refinement_stopped():
    ...
    m = coupled()
    r = sens_solution_report(m, [(m.p, -1.2)], mode="fix_relax")
    ...
    for mode in ("linear", "path"):
        assert sens_solution_report(m, [(m.p, -1.2)], mode=mode).refine_stop is None
...
def test_settled_means_nothing_is_left_outside_a_bound(mk, target):
    ...
    m = mk()
    r = sens_solution_report(m, [(m.p, target)], mode="fix_relax")
...
def test_bound_eps_sets_what_counts_as_leaving_a_bound():
    ...
    m = coupled()
    plain = sens_solution(m, [(m.p, -1.2)], mode="linear", clamp=False)
    tight = sens_solution(m, [(m.p, -1.2)], mode="fix_relax", bound_eps=1e-9)
    slack = sens_solution(m, [(m.p, -1.2)], mode="fix_relax", bound_eps=10.0)
...
def test_bound_eps_decides_crossed_and_the_stop_reason_together():
    ...
    for eps in (1e-9, 1e-2, 1.0, 10.0):
        r = sens_solution_report(m, [(m.p, -1.2)], mode="fix_relax", bound_eps=eps)
        assert r.refine_stop == "settled"
        assert list(r.crossed) == [], (
...
def test_bound_eps_leaves_constraint_rows_alone():
    ...
    m = with_row()
    r = sens_solution_report(m, [(m.p, 3.0)], mode="fix_relax", bound_eps=10.0)
...
def test_the_margin_is_absolute_as_the_refinement_is():
    ...
    r = sens_solution_report(m, [(m.p, target)], mode="linear")
    ...
    with pytest.warns(UserWarning, match="clamped"):
        sens_solution(m, [(m.p, target)], mode="linear")
...
def test_bound_eps_unset_is_the_solves_own_margin():
    ...
    unset = sens_solution(m, [(m.p, -1.2)], mode="fix_relax")
    floor = sens_solution(m, [(m.p, -1.2)], mode="fix_relax", bound_eps=1e-9)
    wide = sens_solution(m, [(m.p, -1.2)], mode="fix_relax", bound_eps=10.0)
...
def test_bound_eps_warns_where_no_refinement_reads_it():
    ...
    for mode in ("linear", "path"):
        with pytest.warns(UserWarning, match="bound_eps"):
            sens_solution(m, [(m.p, -1.2)], mode=mode, bound_eps=1e-3)
        with pytest.warns(UserWarning, match="bound_eps"):
            sens_solution_report(m, [(m.p, -1.2)], mode=mode, bound_eps=1e-3)
...
def test_bound_eps_takes_the_option_surfaces_bounds(bad):
    ...
    m = coupled()
    for call in (sens_solution, sens_solution_report):
...
def test_arguments_are_checked_before_the_factor_is():
    ...
    with pytest.raises(ValueError, match="mode"):
        sens_solution(m, [(m.p, 0.5)], mode="relax_fix", max_pdpert=worst / 10.0)
    with pytest.raises(ValueError, match="mode"):
        sens_solution_report(m, [(m.p, 0.5)], mode="relax_fix",
                        max_pdpert=worst / 10.0)
    with pytest.raises(ValueError, match="degeneracy"):
        sens_active_set_changes(m, [(m.p, 0.5)], degeneracy="one-sided",
                           max_pdpert=worst / 10.0)
...
def test_max_pdpert_refuses_a_factor_the_correction_perturbed():
    ...
    for call in (lambda cap: sens_jacobian(m.x[0], wrt=m.p, max_pdpert=cap),
                 lambda cap: sens_solution(m, [(m.p, 0.5)], max_pdpert=cap),
                 lambda cap: sens_solution_report(m, [(m.p, 0.5)], max_pdpert=cap),
                 lambda cap: sens_active_set_changes(m, [(m.p, 0.5)],
                                                max_pdpert=cap)):
        call(None)
        call(worst * 10.0)
...
def test_max_pdpert_reads_the_same_comparison_the_option_reads():
    ...
    sens_solution(m, [(m.p, 0.5)], max_pdpert=worst)
    with pytest.raises(ValueError, match="max_pdpert"):
        sens_solution(m, [(m.p, 0.5)], max_pdpert=worst * (1.0 - 1e-12))
...
def test_max_pdpert_takes_the_option_surfaces_bounds(bad):
    ...
    with pytest.raises(ValueError,
        ...
        sens_solution(m, [(m.p, 0.5)], max_pdpert=bad)
