"""of= block selection (covariance roadmap item 3): both accessors
reduce onto any block of the solve's variables off the held factor,
post-solve. The declared fitted block is the default, so the no-of=
...
refusal from sens_information(); strongly active variables outside the
...
def test_of_default_is_the_fitted_block():
    # of=[the fitted block, in order] must be EXACTLY the no-of=
    # answer, both accessors: same matrix, same keys
    m, X = solved()
    cov0 = sens_covariance(m, sigma_sq=SIGMA**2)
    cov1 = sens_covariance(m, sigma_sq=SIGMA**2, of=[m.a, m.b])
    np.testing.assert_array_equal(cov0.matrix, cov1.matrix)
    info0 = sens_information(m)
    info1 = sens_information(m, of=[m.a, m.b])
...
def test_of_subblock_is_the_marginal():
    # of=[m.a] is the marginal over b: element 00 of the full
    ...
    gn_a = sens_covariance(m, sigma_sq=SIGMA**2, hessian="gauss-newton",
                      of=[m.a])
    assert gn_a[m.a] == pytest.approx(C[0, 0], rel=1e-9)


def test_of_rank_deficient_block_is_the_prediction_band():
    ...
    with pytest.raises(RuntimeError, match="rank-deficient"):
        sens_information(m, of=m.r)
    with pytest.raises(RuntimeError, match="rank-deficient"):
        sens_covariance(m, sigma_sq=SIGMA**2, hessian="gauss-newton", of=m.r)


def test_of_conditioned_on_reports_the_outside_active_set():
    ...
    # an of= block that is ENTIRELY pinned: information is the Schur
    # onto it (S with nothing free), covariance is the zero row
    R = 2.0 * X.T @ X
    S_a = R[0, 0] - R[0, 1] ** 2 / R[1, 1]
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        info_pin = sens_information(m, of=[m.a])
        cov_pin = sens_covariance(m, sigma_sq=SIGMA**2, of=[m.a])
    assert info_pin[m.a] == pytest.approx(S_a, rel=1e-9)
    assert cov_pin[m.a] == 0.0


def test_of_accepted_forms():
    # a whole IndexedVar, a slice, a (Var, iterable) pair, and a mixed
    # list all normalize to the same coordinates
    m, X = solved()
    whole = sens_covariance(m, sigma_sq=SIGMA**2, of=m.r)
    sliced = sens_covariance(m, sigma_sq=SIGMA**2, of=m.r[:])
    paired = sens_covariance(m, sigma_sq=SIGMA**2, of=(m.r, range(N)))
    np.testing.assert_array_equal(whole.matrix, sliced.matrix)
    np.testing.assert_array_equal(whole.matrix, paired.matrix)
    mixed = sens_covariance(m, sigma_sq=SIGMA**2, of=[m.a, m.r[0]])
    ...
    tup = sens_covariance(m, sigma_sq=SIGMA**2, of=(m.a, m.b))
    lst = sens_covariance(m, sigma_sq=SIGMA**2, of=[m.a, m.b])
    assert tup.matrix.shape == (2, 2)
    np.testing.assert_array_equal(tup.matrix, lst.matrix)


def test_of_derived_sigma_uses_the_fits_degrees_of_freedom():
    ...
    m, X = solved()
    cov_full = sens_covariance(m)
    cov_a = sens_covariance(m, of=[m.a])
    assert cov_a[m.a] == cov_full[m.a]
    assert cov_a.sigma_sq == cov_full.sigma_sq


def test_of_conditioned_on_is_scale_invariant():
...
def test_of_subblock_schur_is_exact_with_a_pinned_member():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        info = sens_information(m, of=[m.a, m.c])
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        gn = sens_information(m, hessian="gauss-newton", of=[m.a, m.c])
    assert gn[m.a] == pytest.approx(R_B[0, 0], rel=1e-9)
    assert gn[m.c] == pytest.approx(S_c, rel=1e-9)


def test_of_exact_under_objective_scaling():
    # gradient-based scaling engaged (df != 1, asserted): the of=
    ...
    assert sens_covariance(m, sigma_sq=sig2, of=[m.a])[m.a] == pytest.approx(
        sig2 * C[0, 0], rel=1e-9)
    band = sens_covariance(m, sigma_sq=sig2, of=m.r)
    np.testing.assert_allclose(band.matrix, sig2 * (X @ C @ X.T),
                               rtol=1e-8, atol=1e-12)
    assert sens_information(m, of=[m.a])[m.a] == pytest.approx(
        2.0 / C[0, 0], rel=1e-9)
    ...
    assert sens_covariance(m2, sigma_sq=sig2,
                      of=[m2.b]).conditioned_on == (m2.a,)


def test_of_with_a_fixed_variable():
...
def test_of_grouped_band_refuses():
    ...
    declare_sens_fitted(m.a)
    declare_sens_fitted(m.b)
    declare_sens_residual(m.r1, group="lo")
    declare_sens_residual(m.r2, group="hi")
    pyo.SolverFactory("pounce").solve(m)
    block = list(m.r1.values()) + list(m.r2.values())
    with pytest.raises(RuntimeError, match="rank-deficient"):
        sens_covariance(m, of=block)


def test_of_binding_row_falls_back_smoothly():
    ...
    # with of= given, the shared diagnostics speak block-relative
...
def test_of_within_count_dependent_block():
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        cov = sens_covariance(m, sigma_sq=SIGMA**2, of=[m.r[0], m.r[1]])
    ...
    with pytest.raises(RuntimeError,
        ...
        sens_information(m, of=[m.r[0], m.r[1]])


def test_of_error_paths():
    m, X = solved()
    with pytest.raises(ValueError, match="twice"):
        sens_covariance(m, sigma_sq=SIGMA**2, of=[m.a, m.a])
    with pytest.raises(TypeError, match="not names"):
        sens_covariance(m, sigma_sq=SIGMA**2, of="a")
    with pytest.raises(TypeError, match="sens_covariance: of= element 5"):
        sens_covariance(m, sigma_sq=SIGMA**2, of=5)
    with pytest.raises(ValueError, match="empty block"):
        sens_covariance(m, sigma_sq=SIGMA**2, of=[])
    m2 = pyo.ConcreteModel()
    m2.q = pyo.Var(initialize=0.0)
    with pytest.raises(ValueError, match="not a variable of the solved"):
        sens_covariance(m, sigma_sq=SIGMA**2, of=[m2.q])
    ...
    with pytest.raises(ValueError, match="removed from the solve"):
        sens_covariance(m3, sigma_sq=SIGMA**2, of=[m3.dead])
