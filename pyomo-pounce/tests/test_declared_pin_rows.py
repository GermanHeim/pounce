...
def test_repeated_solves_construct_no_interface(si_counter):
    ...
    for val in (1.0, 2.0, 0.5):
        ...
        est = sens_solution(m, [(m.p, val + 0.1)])
...
def test_orientation_and_offset_forms_conform(si_counter):
    ...
    assert sens_jacobian(m.w, wrt=m.q) == pytest.approx(1.0, abs=1e-6)
    est = sens_solution(m, [(m.p, 1.2), (m.q, 0.9)])
...
def test_sequential_rewrites_each_get_their_block():
    ...
    assert sens_jacobian(m.x, wrt=m.p) == pytest.approx(-0.5, abs=1e-5)
...
def test_a_param_in_the_objective_takes_the_rewrite(si_counter):
    ...
    with pytest.warns(UserWarning, match="rewritten in place"):
        declare_sens_param(m.p)
    pyo.SolverFactory("pounce").solve(m)
    assert sens_jacobian(m.x, wrt=m.p) == pytest.approx(1.0, abs=1e-5)
...
def test_a_declared_fixed_var_tracks_re_solves(si_counter):
    ...
    assert sens_jacobian(m.x, wrt=m.u) == pytest.approx(3.0, abs=1e-6)
    m.u.set_value(5.0)
    opt.solve(m)
...
def test_indexed_conforming_params_solve_as_written(si_counter):
    ...
    assert m.component(sens_mod._DEFS) is None
    for i in range(3):
        assert sens_jacobian(m.x[i], wrt=m.p[i]) == pytest.approx(1.0, abs=1e-6)
    assert sens_jacobian(m.x[0], wrt=m.p[1]) == pytest.approx(0.0, abs=1e-8)
    est = sens_solution(m, [(m.p, {0: 1.3, 1: 0.7, 2: 1.0})])
...
def test_a_bound_only_param_is_rewritten_without_the_warning():
    ...
    assert sens_jacobian(m.x, wrt=m.hi) == pytest.approx(1.0, abs=1e-5)
...
def test_declared_and_call_time_params_share_a_solve(si_counter):
    ...
    est = sens_solution(m, [(m.p, 1.4), (m.q, 1.0)])
