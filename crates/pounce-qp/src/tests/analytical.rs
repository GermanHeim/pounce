...
// ─────────────────────────────────────────────────────────────────
// Unsupported-feature path: a QP with finite variable bounds should
// be rejected with QpError::UnsupportedFeature until the working-
// set machinery lands. This is a contract test, not a math test.
// ─────────────────────────────────────────────────────────────────
#[test]
fn rejects_bounded_qp_with_unsupported_feature_error() {
    let n = 2;
    let h = identity_hessian(n);
    let a = empty_gen(0, n);
    let g = [0.0, 0.0];
    let bl: [f64; 0] = [];
    let bu: [f64; 0] = [];
    let xl = [0.0, 0.0]; // finite ⇒ requires working set
    let xu = [1.0, 1.0];
