...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_lower_bounded_number_option("partitioned_curvature_cap", "Multiple of an element's own implied curvature that a single quasi-Newton update to its block may reach.", 0.0, true, 1e1, "Only used when \"hessian_approximation\" is \"partitioned\". An element's secant pair implies a curvature of ||y_e||/||s_e||; this caps how far one update may move that element's block, in those same units. It is the backstop the relative denominator floors do not provide: they bound the ratio that produced the rank-1 term, this bounds the term in the units of the quantity being modelled. Measured on benchmarks/large_scale laptime, without it the implied element curvature stayed a healthy 9-33 every iteration while single-update block changes reached 1.7e8 and the assembled Hessian ran four orders of magnitude over the exact one.")?;
