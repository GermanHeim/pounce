...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_bool_option("feral_refine", "Whether FERAL should run iterative refinement on every back-solve.", true, "Iterative refinement at solve time closes the residual floor produced by cascade-break's L-factor perturbation. Enabled by default. IMPORTANT (pounce gh#698): on the IPM path this loop is nested inside PdFullSpaceSolver's own iterative refinement, which is Ipopt's and is itself capped at max_refinement_steps (10). FERAL's own inner cap is 10 correction steps plus the initial solve, so one augmented-system solve could run up to 10x11 substitution passes, each preceded by a matvec against the original matrix; and on a large ill-conditioned KKT the inner loop reaches that cap on every back-solve, because its convergence target is machine precision and is unreachable there. Only the outer loop computes the residual ratio and decides whether the answer is good enough; the inner one drives a residual nobody consults to a tolerance nobody set. Measured on a 118276-dimension KKT: setting this to no cut LinearSystemBackSolve by 60% (147.3 s to 58.3 s) and wall time by 20% (487 s to 388 s), still converged to Optimal, and reached an objective closer to the MA57 reference than the refined run did. The inner cap is no longer hard-coded upstream (feral#178, shipped in feral 0.17.0), so this knob is no longer all-or-nothing: feral_refine_steps caps the inner loop without leaving the refined entry point, but no smaller constant is safe as a global default, for the reasons recorded there -- Ipopt disables backend-internal refinement on every direct solver it ships, because the backend can only refine the condensed system it factorized while Ipopt refines the unreduced one, and what stops pounce doing the same is FERAL's ForceAccept zero-pivot default. It is left enabled by default, though not on the grounds it used to stand on: pinene_3200 was said to stall in the IPM tail without it and had never been re-measured, and when measured (gh#710) it does not stall -- at 64000 variables it reaches the same objective in 12 iterations with this set to no. See feral_refine_steps for the corpus sweep behind that default. Set no when back-solve dominates the timing report (a direct solver spending longer back-substituting than factorizing is the signature). Neither Ma57SolverInterface nor TSymLinearSolver has in-backend refinement, so this duplication is FERAL-only.")?;
