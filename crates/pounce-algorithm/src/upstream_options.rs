...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_bool_option("feral_refine", "Whether FERAL should run iterative refinement on every back-solve.", true, "Iterative refinement at solve time closes the residual floor produced by cascade-break's L-factor perturbation; without it, pinene_3200 and similar IPM tails stall as the per-pivot residual exceeds the duality gap. Enabled by default. IMPORTANT (pounce gh#698): on the IPM path this loop is nested inside PdFullSpaceSolver's own iterative refinement, which is Ipopt's and is itself capped at max_refinement_steps (10). FERAL's inner loop is capped at 10 correction steps plus the initial solve, so one augmented-system solve can run up to 10x11 substitution passes, each preceded by a matvec against the original matrix. Only the outer loop computes the residual ratio and decides whether the answer is good enough; the inner one drives a residual nobody consults to a tolerance nobody set. Measured on a 118276-dimension KKT: setting this to no cut LinearSystemBackSolve by 60% (147.3 s to 58.3 s) and wall time by 20% (487 s to 388 s), still converged to Optimal, and reached an objective closer to the MA57 reference than the refined run did. It is left enabled by default because the pinene_3200 case above has not been re-measured against it and is not in the fixture corpus -- the accuracy case for the inner loop is not visible in the gh#698 results, but neither is its absence tested on the case that motivated it. Set no when back-solve dominates the timing report (a direct solver spending longer back-substituting than factorizing is the signature). Neither Ma57SolverInterface nor TSymLinearSolver has in-backend refinement, so this duplication is FERAL-only.")?;
