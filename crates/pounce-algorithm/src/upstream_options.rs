...
pub fn register_all_upstream_options(r: &RegisteredOptions) -> Result<(), SolverException> {
    ...
    r.add_bool_option("feral_refine", "Whether FERAL should run iterative refinement on every back-solve.", true, "Iterative refinement at solve time closes the residual floor produced by cascade-break's L-factor perturbation. Enabled by default. IMPORTANT (pounce gh#698): on the IPM path this loop is nested inside PdFullSpaceSolver's own iterative refinement, which is Ipopt's and is itself capped at max_refinement_steps (10). FERAL's inner loop is capped at 10 correction steps plus the initial solve, so one augmented-system solve can run up to 10x11 substitution passes, each preceded by a matvec against the original matrix. Only the outer loop computes the residual ratio and decides whether the answer is good enough; the inner one drives a residual nobody consults to a tolerance nobody set. Measured on a 118276-dimension KKT: setting this to no cut LinearSystemBackSolve by 60% (147.3 s to 58.3 s) and wall time by 20% (487 s to 388 s), still converged to Optimal, and reached an objective closer to the MA57 reference than the refined run did. The inner cap is no longer hard-coded upstream (feral#178, shipped in feral 0.17.0), so this knob is no longer all-or-nothing: feral_refine_steps caps the inner loop without leaving the refined entry point, and one correction step rather than ten is the setting expected to make the default worth revisiting (gh#710). It is left enabled by default, though not on the grounds it used to stand on: pinene_3200 was said to stall in the IPM tail without it and had never been re-measured, and when measured (gh#710) it does not stall -- at 64000 variables it reaches the same objective in 12 iterations with this set to no. What does object to a reduced budget is the fixture corpus; see feral_refine_steps. Set no when back-solve dominates the timing report (a direct solver spending longer back-substituting than factorizing is the signature). Neither Ma57SolverInterface nor TSymLinearSolver has in-backend refinement, so this duplication is FERAL-only.")?;
    r.add_lower_bounded_integer_option("feral_refine_steps", "Maximum correction steps in FERAL's inner iterative refinement, per back-solve.", 0, 10, "FERAL's inner iterative-refinement budget (feral#178, requested by pounce gh#698 observation 5 and tracked as gh#710). Only consulted when feral_refine is yes. Ten is FERAL's own default and is right for a caller that solves Ax=b once and keeps the answer; it is wrong for a caller running its own refinement over the same system, which PdFullSpaceSolver does -- it computes the residual after each back-solve and decides from it whether to continue. Nested, one augmented-system solve costs up to max_steps x (max_steps+1) substitution passes plus a matvec against the original matrix per step, and only the outer loop consults the residual it drives. Measured on a 118276-dimension KKT: turning the inner loop off entirely cut LinearSystemBackSolve 60% (147.3 s to 58.3 s) and wall time 20%. Setting 0 is not the same as feral_refine=no -- it still routes through the refined entry point and so through that path's choice of solve core -- so use feral_refine=no to switch refinement off. The cap is an upper bound only: FERAL's eps*sqrt(n) residual target, its 100x divergence guard and its 2-strike plateau exit all keep priority, and the best-iterate contract holds at every value, so no cap can return an answer worse than the unrefined solve. Worth knowing before retuning: feral#179 measured that nothing merely ill-conditioned reaches a 10-step budget at all (Hilbert n=8..40 stops at 3-7, ill-conditioned bordered KKTs at 1); the budget is only reachable when the factor is a genuinely perturbed approximate inverse, which is pounce's case because pounce perturbs the L-factor. Ten remains the default because 1 is not safe as one: pinene_3200 (64000 variables) converges identically at 10, 1, 0 and refine=no, but sweeping the fixture corpus at 1 moves 15 of 118 legs and loses two of them -- deb7 on the exact-Hessian leg goes from SolveSucceeded in 143 iterations to ErrorInStepComputation in 183, and cresc4 under limited-memory goes from SolveSucceeded in 99 iterations to InfeasibleProblemDetected in 32. Other legs improve (pooling_rt2stp 298 -> 199). Treat a lowered cap as a per-problem tuning lever, not a global one, and re-check the answer.")?;
