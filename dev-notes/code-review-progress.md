...
| L26 | CLI summary (`print.rs`): (a) the final-summary block prints the **same** number under both the `(scaled)` and `(unscaled)` columns for Dual infeasibility / Constraint violation / Complementarity / Overall NLP error (`print.rs:381-401`); (b) "Variable bound violation" is hardcoded `0.0` (`print.rs:391`); (c) the inequality bound-type breakdown didn't sum to `n_ineq` — a "free" inequality row (no finite bound) fell through to a no-op arm (`print.rs:87-89`) | **FIXED** (c fixed + tested; b fixed + tested via gh#900; a fixed with the unscaled residual pairs) | **All three verified by reading code.** **(c) FIXED:** a constraint row with `g_l=-inf, g_u=+inf` is classified inequality (`n_ineq += 1`) but its `(has_l,has_u)=(false,false)` match arm was `=> {}`, so `ineq_lower_only + ineq_both + ineq_upper_only < n_ineq` whenever a free row was present — exactly the "breakdown not sum to total" defect. The comment already said to "count it anyway under 'both' to keep the totals consistent," so the code was simply not doing what its comment claimed. **Fix**: `(false, false) => ineq_both += 1`, comment rewritten to match. **Test** (`print.rs` `inequality_tally_tests::free_inequality_row_keeps_breakdown_summing_to_total`): a mock TNLP with 3 inequality rows (lower-only, both, free) asserts `ineq_lower_only + ineq_both + ineq_upper_only == n_ineq == 3` (and the free row lands in "both": both=2). **Fail-first confirmed**: reverting to `=> {}` makes the sum 2 ≠ 3 (test fails). 160 pounce-cli lib tests green; `cargo fmt` / `cargo clippy -p pounce-cli --bin pounce -- -D clippy::correctness -D clippy::suspicious` clean. **(b) FIXED (gh#900); (a) fixed later with the unscaled residuals. At the time of this review both were classed as missing solver statistics rather than print bugs:** `SolveStatistics` (`pounce-nlp/src/solve_statistics.rs:51-66`) carries a single value each for `final_dual_inf`/`final_constr_viol`/`final_compl`/`final_kkt_error` (populated once off the scaled-space `cq` cache at `application.rs:1347-1361`) and **no** variable-bound-violation field; only the objective is tracked in both spaces (`final_objective` + `final_scaled_objective`). So the print layer has only the scaled residual to show — printing it under both columns is the *display* symptom of the solver not computing unscaled residuals, and the `0.0` bound violation is a value the solver never measures. **That parenthetical — "an interior-point iterate is bound-feasible by construction, so it is ~0 on a converged solve" — is false, and gh#900 is the correction:** the iterate is feasible for the box the solver was *handed*, which `bound_relax_factor` has widened, and the row is about the box the caller *declared*. A converged solve with an active bound sits one full widening outside it. On `min 1e8x + ½x² s.t. x >= 0` the row printed `0.00e+00` while the returned objective was `-0.99999` for a quantity whose true minimum is `0`; upstream Ipopt prints `9.9999090909090909e-09` there. Now measured on both arms (`curr_declared_box_violation_max` / `QpResiduals::bound_violation`) and carried as `final_declared_box_viol`; see `## L26 detail` and `crates/pounce-cli/tests/issue_900_variable_bound_violation_row.rs`. A faithful fix requires plumbing unscaled residuals + a bound-violation metric through `pounce-algorithm`'s finalize path into `SolveStatistics` — a solver-statistics change well beyond a CLI print remediation, deferred to a dedicated issue. Nothing parses this block (grep over `crates/`/`pyomo-pounce/`/`python/` finds no scraper) — read at the time as "cosmetic, not a data-integrity break", which is the inference gh#900 overturned: the consumer of this block is a human deciding whether to trust an answer, and no grep finds them. See `## L26 detail`. |
...
### (b) Variable bound violation hardcoded `0.0` — FIXED (gh#900)

`row("Variable bound violation", 0.0, 0.0)` (then `print.rs:391`, latterly
`pounce-solve-report/src/console.rs:499` and `:695`). pounce's
`SolveStatistics` had no bound-violation field and the print site was not handed
the final `x` or the variable bounds, so the value was not derivable there
without plumbing the metric through the solver statistics. Deferred, at the
time, to the same solver-statistics issue as (a).

**The deferral reasoning was wrong, and the way it was wrong is the lesson.**
The note read: "For a *converged* interior-point iterate the true value is ~0
by construction (slacks stay strictly positive), so the constant is
approximately right at optimum but genuinely uncomputed for early-terminated
solves." The slacks stay strictly positive against the box the solver was
**handed**, which `bound_relax_factor` has widened by `min(factor, cap)·max(|b|, 1)`.
Against the box the caller **declared** — the one the row is about — a converged
iterate sits up to one full widening outside, every time a bound is active. So
the constant was not "approximately right at optimum": it was wrong at optimum
and right nowhere, and the class it was wrong on is precisely the class the row
exists to expose. On `min 1e8·x + ½x² s.t. x ≥ 0` (analytic `x* = 0`, `f* = 0`)
the NLP arm returns `f = -0.99999090909` under `Optimal Solution Found` and used
to print `0.00e+00` here; upstream Ipopt prints `9.9999090909090909e-09`.

Two things had to be true for that to slip through, and both are general: the
argument reasoned about an *invariant of the algorithm* ("slacks stay positive")
without naming **which model** the invariant is relative to, and it was checked
against a corpus with nothing on it that presses hard on a bound — the same
uniform-corpus failure the fixture-sweep notes in `CLAUDE.md` keep recording.

Fixed by `IpoptCalculatedQuantities::curr_declared_box_violation_max` (NLP arm,
over `Nlp::declared_box_violation`, which arrived later for
`honor_original_bounds` and gh#744/#745) and `QpResiduals::bound_violation`
measured against a `BoundRelax::NONE` re-extraction (convex arm), surfaced as
`SolveStatistics::final_declared_box_viol`. Regression test:
`crates/pounce-cli/tests/issue_900_variable_bound_violation_row.rs`, four
corners — both arms × widened/unwidened — because the two arms compute the
number by wholly independent paths and a zero that must not be fabricated is as
much of a defect as a zero that hides a violation.

Note that (a) is now also fixed independently: the residual rows carry genuine
scaled/unscaled pairs. The box violation is the one row that legitimately
prints the same number twice — variable bounds carry no scaling, because pounce
scales the objective and the constraint rows only.
...
so (a)/(b) were classed as cosmetic rather than data-integrity problems, and (c)
as the only behavioral defect in L26.

That classification held for the machine and not for the reader, which gh#900
is the correction to: "no parser consumes it" is not "nothing depends on it"
when the block is the documented place a user checks whether their answer is
feasible. A row that always says `0.00e+00` is not an imprecise display of a
number; it is a false statement about the answer, and the user acting on it is
the consumer the grep could not find.
