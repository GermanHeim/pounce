# Spatial branch-and-bound (`pounce-global`) — design record

> **Status: removed from `main`. Not shipped, not on any branch.**
>
> This crate was developed on the PR #70 line and deleted on 2026-06-07 by
> commit `9a4c908d` ("refactor: strip pounce-global from the LP/QP merge
> path"), so that the convex / LP / QP half of that work could merge without
> it. `9a4c908d` *is* an ancestor of `main`, which is why the repository still
> carried documentation and benchmarks for a crate no branch contains.
>
> The source — ~8,200 lines including the Rust crate, the `pounce-py`
> bindings, and `python/tests/test_global.py` — is recoverable from history:
>
> ```sh
> git show 9a4c908d^:crates/pounce-global/src/relax.rs
> git checkout 9a4c908d^ -- crates/pounce-global   # restore the whole crate
> ```
>
> This note is the design rationale, moved verbatim out of
> `docs/src/global-optimization.md` on 2026-08-09 so that the user-facing
> documentation stops describing a solver users cannot call. Nothing below
> has been re-verified against the current workspace; treat every code
> snippet and option name as accurate *as of `9a4c908d^`* only.
...
As of `9a4c908d^`, `pounce-global` was a complete, correct *continuous* global
solver. It never reached commercial-solver scale, and was never wired into a
shipped release:
